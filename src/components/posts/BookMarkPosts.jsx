...
const BookMarkPosts = () => {
  ...
  //fetch from store
  const likePosts = useSelector((state) => state.likes);
  const allPosts = useSelector((state) => state.posts); 
  const bookMarkPosts = useSelector((state) =>
    state.bookMarks.map((postId) => allPosts.find((post) => post.id === postId))
  );
  ...
  const handleBookMark = (post) => {
    if (bookMarkPosts.find((p) => p.id === post.id)) {
  ...
  const handleDeletePost = (id) => {
    const isBookmarked = bookMarkPosts.find((post) => post.id === id);
                ...
                {filteredPosts.map((post, index) => (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center justify-start space-x-2">
                      ...
                      <span>{likePosts.includes(post.id) ? "1" : "0"}</span>
                    </div>
                  </div>
                ))}
              </div>
